...
def prepare_env_file(
    ...
    if source_path.exists():
        ...
        logger.warning(f"Source .env file not found at {source_env_path}, creating default config...")
...
def stop_existing_containers(project_name: str = "supabase") -> None:
    """
...
def start_supabase(project_name: str = "supabase", detached: bool = True) -> None:
    """
    ...
    cmd = [
        "docker", "compose", "-p", project_name,
        "-f", "supabase/docker/docker-compose.yml", "up"
    ]
...
def check_supabase_status(project_name: str = "supabase") -> bool:
    """
    ...
    try:
        result = run_command([
            "docker", "compose", "-p", project_name,
            "-f", "supabase/docker/docker-compose.yml", "ps", "--format", "json"
        ])
...
def main() -> None:
    ...
    try:
        stop_existing_containers(args.project_name)
    except Exception as e:
    ...
    # Start Supabase
    try:
        start_supabase(args.project_name, not args.no_detach)
    except Exception as e:
    ...
    if check_supabase_status(args.project_name):
        logger.info("""
